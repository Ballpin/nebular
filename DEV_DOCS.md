...
- src
    ...
    - framework - Framework itself, divided into npm packages
        - theme - `nebular/src/framework/theme` npm package, main framework package
        - auth - `nebular/src/framework/auth` npm package, auth package (login, register, etc)       
        - icons - `nebular-icons` npm package, cool icons font
        - security - `nebular/src/framework/security` npm package, security framework package
...
- Solution
  ...
  - SCSS MAP
    ...
    ```
      @import 'nebular/src/framework/theme/styles/component';
...
Documentation is generated by the custom generator built on top of nebular/src/framework/theme.
...
1. `rm -rf node_modules/nebular/src/framework` to remove the package installed
...
4. link Nebular in ngx-admin: `npm link nebular/src/framework/{auth,theme}`
